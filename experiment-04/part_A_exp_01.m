...
timeInterval = 0 : dt : PulseTrainPeriod;
%timeInterval = -PulseTrainPeriod / 4 : dt : 3 * PulseTrainPeriod /4;
%timeInterval = -PulseTrainPeriod : dt : PulseTrainPeriod;n = timeInterval / dt;
n = timeInterval / dt;
L = PulseWidth / dt;
...
NCoefficients = N;
if ~(mod(NCoefficients, 2))
    k = -NCoefficients / 2 : (NCoefficients / 2) - 1;
else
    k = -(NCoefficients - 1) / 2 : (NCoefficients - 1) / 2;
end
c = zeros(1, length(k));
for j1 = 1 : length(k)
